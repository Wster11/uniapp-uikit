...
const unwatchQuoteMsg = autorun(() => {
  ...
  if (props.msgId) {
    msg.value =
      deepClone(EaseConnKit.messageStore.messageMap.get(props.msgId)) || null;
    ...
    quoteFrom.value = EaseConnKit.appUserStore.getUserInfoFromStore(
      msg.value.from
    );
  } else {
    msg.value = deepClone(EaseConnKit.messageStore?.quoteMessage) || null;
    if (!msg.value) {
      return;
    }
    quoteFrom.value = EaseConnKit.appUserStore.getUserInfoFromStore(
