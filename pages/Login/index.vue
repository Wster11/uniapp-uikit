<template>
	<view class="login-wrap">
		<view class="login-header">
			<image @click="times++" src="/static/logo.png" style="width: 100px; height: 100px"></image>
		...
		<view v-if="isPasswordLogin" class="login-form-warp">
			...
			<view class="input-wrap">
				<input type="password" v-model="password" :placeholder="t('loginPasswordPlaceholder')" />
			</view>
		</view>
		<!--手机号验证码登录  -->
		<view v-else class="login-form-wrap">
			<view class="input-wrap">
				<input v-model="tel" maxlength="11" :placeholder="t('loginPhoneIdPlaceholder')" />
			</view>
			<view class="item-wrap">
				<input v-model="code" maxlength="6" :placeholder="t('loginCodePlaceholder')" />
				<button size="mini" type="primary" class="send-code easeim-btn" @tap="getCode"
					:disabled="counter !== 60">
